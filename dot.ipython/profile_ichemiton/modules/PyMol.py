...
class MolViewer(object):
  ...
  def ShowMol(self,mol,name='/tmp/molecule',showOnly=True,highlightFeatures=[],
    ...
    mol.write('pdb', name, overwrite=True)
    mid = server.do('load %s' % name)
...
#    fd = open('/tmp/asd.png', 'w')
#    fd.close()
    self.server.do('ray')
    self.server.do('png %s'%fd.name)
    time.sleep(1)  # <- wait a short period so that PyMol can finish
